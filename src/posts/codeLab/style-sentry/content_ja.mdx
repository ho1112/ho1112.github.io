...
### 元の目的: 未使用CSSクラス整理
Style Sentryを作った本来の目的である未使用CSSクラス検出でもよく使用しています。チームプロジェクトで時間が経つにつれて蓄積された不要なスタイルを体系的に整理でき、コードベースがずっとクリーンになりました。

### もう一つの活用方法: コンポーネントリファクタリング

しかし開発過程で最も有用だったのは**コンポーネント分離作業**でした。

大きなコンポーネントを複数の小さなコンポーネントにリファクタリングする時、JSXは比較的簡単に分離できますがSCSSファイル分離は厄介な作業でした。既存に一つのSCSSファイルにすべてのスタイルが入っているのに、新しく分離したコンポーネントで実際に使用するクラスが何なのか把握するのが難しかったのです。

手動で一つずつ確認するには時間が長くかかり、間違いも起こしやすかったです。

**Style Sentryを活用:**
1. 既存SCSSファイルを新コンポーネントフォルダにコピー
2. 警告が表示されるクラスのみ削除
3. 必要なスタイルのみ残して整理完了

最初は単純に未使用クラスを見つけてくれるコード整理ツール程度に考えていましたが、大規模コンポーネントを小さな単位に分離するリファクタリング作業にも活用して時間を大幅に節約することができました。 👍
