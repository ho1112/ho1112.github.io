---
title: 'Dead Internet Theory - AIペルソナボットコメント自動化システム開発記'
...
# Dead Internet Theory - AIペルソナボットコメント自動化システム開発記

## 1. 概要
正直に言うと、私のブログのコメント欄はほとんど「開店休業」状態でした。独り言を言っている気分になる頃、興味深い仮説を一つ知りました。いわゆる Dead Internet Theory（死んだインターネット理論）です。

**死んだインターネット理論**とは、私たちが見ているウェブの大部分が実際の人ではなくAIボットが生成したコンテンツで埋め尽くされている、という少し不気味な仮説です。（最近YouTubeでも、他人のコメントをそのままなぞったり、コメントがないのにリアクションだけ乱発する投稿を見かけませんか？👿）

「もしかしたら未来は本当にこうなるのかも？」という好奇心から、すぐに手を動かしました。さびしいコメント欄を個性豊かなAIペルソナ達で賑やかにする一方で、この機会に外部サービス（Giscus）への依存も断ち切り、自分だけの独立したコメントシステムを作ることにしました。

とはいえ、すべての記事がボットで埋まってしまうのはそれはそれで怖いので、まずは weeklyニュースレターのまとめ記事にのみ適用するつもりです。ほかの記事は私自身の考えを綴る場でもあるので、AIの多様な視点までは不要だと思っています。
...
#### Heisenbug
<img src="https://dead-internet-theory-sigma.vercel.app/personal/Heisenbug.svg" alt="ハイゼンバグ" width="75" />
...
ニックネームは未来学者レイ·カーズワイルの著書「(The Singularity Is Near)」から持ってきました。
#### wow!
...
### 2025年12月20日追加対応
Google AI Studioのポリシー変更により無料APIの使用が制限されました。既存の2.5-proモデルの使用が制限され、コメント生成が失敗していました。
APIリクエスト方式も変更されたため、併せて対応しました。

既存: fetch方式、2.5-proモデル使用
```typescript
    // 3. Gemini API呼び出し
    ...
    if (!response.ok) {
      throw new Error(`Gemini API呼び出し失敗: ${response.status}`);
    }
...
修正: @google/genai認証方式に変更、3.0 flashモデルに変更
```typescript
import { GoogleGenAI } from '@google/genai';

   // 3. Gemini API呼び出し (新しいSDK使用)
...
```
[変更PR](https://github.com/ho1112/dead-internet-theory/pull/14/files)
