...
<div style={{ display: 'flex', gap: '2px', flexWrap: 'wrap', justifyContent: 'center', margin: '24px 0' }}>
  <img
    src="/posts/codeLab/cha-line/toss.avif"
    style={{
      maxWidth: '100%',
      height: 'auto',
      flex: '1',
  ...
  />
  <img
    src="/posts/codeLab/cha-line/kakao.avif"
    style={{
      maxWidth: '100%',
      height: 'auto',
      flex: '1',
...
<small>※ 한국처럼 직접 푸쉬메시지로 알려주는 것을 원했습니다.</small>
...
### LINE Flex 메시지로 변환
LINE Flex 메시지는 LINE이 제공하는 리치 메시지 형식으로 레이아웃, 스타일, 버튼 등을 JSON 형식으로 자유롭게 구성할 수 있습니다. 일반 텍스트 메시지보다 시각적이고 정보 전달이 용이하여 배당금 내역처럼 구조화된 데이터를 보기 좋게 표시하기에 적합합니다.
[LINE Flex Message란?](https://developers.line.biz/ja/docs/messaging-api/using-flex-messages/)
공식계정에서 받는 LINE메시지를 상상하시면 됩니다!
...
LINE Flex메시지의 시뮬레이터도 제공하고 있기 때문에 프리뷰를 보면서 작업할 수도 있습니다.
![](/posts/codeLab/cha-line/flex-message-simulator-ja.avif)
[Flex Message Simulator](https://developers.line.biz/flex-simulator/)(LINE Developers로그인이 필요합니다!)
...
### 이름 정하기 & 라인 공식 계정 프로필?
라인의 공식게정과 리포지토리 이름을 어떻게 해야할지 고민하다가 일본어로 동전이 떨어지는 소리인 **チャリン**과 LINE을 합쳐서 `チャライン`으로 정했습니다.

모티브는 바로 이 포켓몬입니다!
<img src="/posts/codeLab/cha-line/치렁.webp" alt="치렁" width='300' />
**치링(チリーン)** - 풍경을 모티브로 만들어진 포켓몬인데, 이 포켓몬과 달러💵를 합쳐서 AI로 캐릭터를 만들었습니다.

<img src="/posts/codeLab/cha-line/chaLine_logo.png" alt="cha-line logo" width='300' />

그런데 나중에 생각해보니 `ちゃらい`와 발음이 비슷해서 바꿔야 하나 생각도 했는데 다른 좋은 이름이 생각 안 나서 일단 유지!
...
9월 1일, 15:07분 배당금 입금 메일이 왔습니다. 몇 분 뒤 GAS에서 이메일을 감지하고 배당금 내역을 LINE에 통지해줄거라고 기대했습니다만...

실패했습니다!
원인은 인증코드 화면에 새로운 피싱 주의 팝업이 추가되었기 때문이었습니다.
![](/posts/codeLab/cha-line/fishing.avif)
화면 전체에 오버레이 되는 것이기 때문에 클릭하기 전까진 인증코드를 입력할 수 없어서 실패했던 것입니다.
그래서 이 팝업을 클릭하는 처리를 playwright에 추가하고 수동으로 다시 시도해주었습니다.
...
9월 8일 15:25분 새로운 배당금 입금 메일이 왔습니다.
<img src="/posts/codeLab/cha-line/250908_line.avif" alt="250908LINE通知完了" width='300' />
15:32분 LINE에 통지 완료.
이번엔 문제없이 한 번에 처리가 완료 되었습니다. 🎉

이후 9월에 2차례정도 더 배당금 알림이 있었습니다. 각각 2~3번 재시도 끝에 LINE 알림 전송에 성공했습니다.
LINE 통지가 이루어지지 않으면 gmail에 라벨을 설정하지 않아 자동으로 re-try하도록 만들어져 있기 때문에 기다리다 보면 알림을 받아볼 수 있었습니다.
...
10월 7일의 통지에서는 실패 없이 한 번에 깔끔하게 성공했습니다. 🎊
그 동안 SBI증권의 UI변경도 없었고 코드 수정도 없었기 때문에 아무래도 실행 시점의 네트워크 환경에 영향을 많이 받는 것으로 보입니다.
