---
title: 'SEO의 과거, 현재, 그리고 미래 | 본질의 시대: 기술적 SEO와 사용자 경험 #2'
...
## 챕터 3. [Architecture] 렌더링과 인덱싱: 봇을 위한 설계

성능 최적화가 사용자를 위한 것이라면 렌더링과 인덱싱 최적화는 우리 사이트를 방문하는 검색 봇(Bot)을 위한 배려입니다.

### 1. 렌더링 전략의 다변화 (Next.js 활용)
...
서비스 규모가 커질수록 봇이 하루에 긁어갈 수 있는 페이지의 수, 즉 크롤링 예산이 부족해지는 문제가 발생합니다. 봇이 중요한 페이지를 놓치지 않고 효율적으로 탐색하도록 유도해야 합니다.
