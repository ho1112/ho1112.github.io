...
## Next.js 14 업그레이드 기록: 5개의 관문을 통과한 Docker/Bun 디버깅 일지
...
`package.json`의 버전 숫자 하나를 바꾸는 이 작업이 저는 빠르면 반나절 길어도 하루면 끝날 것이라고 순진하게 믿고 있었습니다. 하지만 제가 마주한 것은 로컬과 다른 Docker 환경, `bun`과 `npm`의 미묘한 차이, 그리고 macOS와 Linux의 근본적인 차이가 빚어내는 거대한 혼돈의 소용돌이였습니다.
...
여기서 리더와 상담을 요청하여 낡고 불안정한 bun.lock 정책을 고수하는 대신 **Bun의 바이너리 방식 bun.lockb를 새로운 표준으로 채택**하는 것을 건의 했습니다. 기존의 bun.lock도 이 방식을 사용해왔을 뿐 무언가 특별한 이유로 유지한 것이 아니었기 때문에 쉽게 변경이 가능했습니다. 이 결정 덕분에 더 이상 `--save-text-lockfile` 같은 복잡한 플래그는 필요 없어졌고 공식 설치 명령어는 간단한 `bun install`이 되었습니다.
...
제가 정립한 최종 아키텍처는 **"로컬에 node_modules와 bun.lockb를 두고 이것을 Docker 컨테이너에 마운트하여 사용한다"** 는 것이었습니다. 이 아키텍처를 유지하면서 의존성을 갱신하는 가장 안정적인 방법을 찾아야 했습니다.

#### **✨ 시행착오 및 해결!**

저는 안정적이고 재현 가능한 최종 워크플로우를 정립하고 README.md에 기록했습니다. 여러 방법을 시도해 본 결과 bun 자체 기능인 `--cwd` 옵션을 사용하는 것이 가장 깔끔했습니다.
...
일주일간의 사투 끝에 저는 마침내 Next.js 14로 성공적으로 업그레이드를 마칠 수 있었습니다. 그 과정에서 "개발 가이드라인"이라고 믿었던 것들이 사실은 현대적인 개발 환경의 발목을 잡는 '기술 부채'일 수 있음을 그리고 그것을 유지하기 위해 얼마나 많은 숨은 노력이 필요한지를 깨달을 수 있었습니다.
...
Next.js 업그레이드를 계획하고 계신다면 package.json의 버전 숫자 외에도 몇 가지를 더 점검해보시길 권합니다. 로컬 환경과 다른 배포 환경의 차이, 사용 중인 패키지 매니저의 세세한 동작 방식, 그리고 팀이나 프로젝트에 암묵적으로 존재하는 오래된 규칙들이 바로 예상치 못한 복병이 될 수 있습니다.
때로는 예상치 못한 문제와 마주하며 보내는 '삽질'의 시간이, 미래를 위한 가장 확실한 투자가 되기도 합니다.
