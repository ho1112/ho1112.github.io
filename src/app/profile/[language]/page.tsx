                ...
                <ul className="text-muted-foreground list-disc list-inside space-y-1">
                  {(language === 'ko'
                    ? [
                        ...
                        '여러 서비스 간의 송객 연계 및, 회원가입·로그인 도입으로 유저 온보딩 최적화',
                        '기업용 대시보드에서 최근 6개월 오퍼 전송·열람·수락 추이를 막대/선 그래프로 시각화하고 전환율 분석 자동화',
                        '기업-학생 면접 일정조정 기능 구축(복수의 일정 첨부, 자동 회신, 참가자의 csv 다운로드 기능)',
                        '컴포넌트 구조 재설계, 리팩토링',
                        '기업 리브랜딩 대응',
                        '생산성 향상 대응(storybook, playwirght, pre-commit hooks)',
                        'Sentry 에러 모니터링 및 버그 수정',
                      ]
                    : [
                        '新卒向けの人材スカウトシステムのtoB・toCシステム開発',
                        '学生管理、検索、ユーザーリスト、オファー、イベント機能追加',
                        '複数サービス間の送客連携および、会員登録・ログイン導線の最適化',
                        '企業向けダッシュボードで直近6か月のオファー送信・開封・承諾推移を棒・折れ線グラフで可視化し、コンバージョン分析を自動化',
                        '企業と学生の面接日程調整機能を開発(複数の日程を添付、自動で返信、参加者のcsvダウンロード機能付き)',
                        'コンポーネント構造の再設計、リファクタリング',
                        '企業リブランディング対応',
                        '生産性向上対応(storybook, playwirght, pre-commit hooks)',
            ...
            <div className="space-y-6">
              ...
              <div className="bg-muted/30 rounded-lg p-5 border-l-4 border-blue-500">
                ...
                <ul className="text-muted-foreground mb-4 list-disc list-inside space-y-1">
                  {(language === 'ko'
                    ? [
                        '외부 서비스로부터의 유입 판정과 인증 상태에 따른 동적 리다이렉트 구현',
                        '기업용 대시보드에서 과거 6개월간의 오퍼 전송·열람·수락·이벤트 참가 지표를 집계하고, 막대/선 그래프로 추이와 전월 대비 증감률을 동시에 보여주는 관리 기능을 개발',
                        '기업-학생 면접 일정조정 기능 설계/구현: 기업이 타이틀·메모·날짜·시간·정원·오프라인 주소/온라인 URL/비밀번호를 등록해 학생에게 제안하고, 학생의 응답(가능 일시 선택/불가)에 따라 기업이 즉시 일정을 재편집하며, 최종 확정된 참가자 명단을 CSV로 다운로드할 수 있는 전체 관리 플로우',
                    ...
                    : [
                        '外部サービスからの流入判定と、認証状態に応じた動的リダイレクトの実装',
                        '企業向けダッシュボードにて直近6か月のオファー送信・開封・承諾・イベント参加指標を集計し、棒/折れ線グラフと前月比で推移を俯瞰できる管理機能を開発',
                        '企業と学生の面接日程調整機能を設計/実装：企業がタイトル・メモ・日付・時間・定員・オフライン会場/オンラインURL・パスワードを登録して学生に提案し、学生の応答(参加可能枠の選択/不可)に応じて企業が即座に日程を再編集し、最終確定した参加者リストをCSVでダウンロードできる全体管理フロー',
